...
//   used normally by the browser, since #/x hashtags search for x.
       ...
       words_to_tag = function (snippets) {for (var i = 0, r = {}, l = snippets.length, s; s = snippets[i], i < l; ++i) s.role === 'source' && $.extend(r, s.index || {}); return r},
          ...
          tag_words = function (html, index) {for (var i = 0, ws = html.split(/(\W+)/), l = ws.length, w; w = ws[i], i < l; i += 2) if (w in index) ws[i] = '<a class="tag" href="#/#{w}">#{w}</a>';
                                              return ws.join('')},

             render = fn[filename, source][let[parsed = process_file(filename, source)] in $(fn_[$('#source').append(render_snippet(words_to_tag(parsed.snippets))(parsed)),
                                         ...
                                         fn_[let[ws = $(this).val() && $(this).val().split(/\W+/)] in
                                             (document.location.hash = '#/#{$(this).val()}',
                                              ...
                                              timeout = setTimeout(fn_[ws && ws.length ?
                                                ($('.snippet').each(fn_[let[i = $(this).snippet().index] in (i && i[ws.join(':')] ? $(this).appear() : $(this).disappear())]),
