...
// snippets. Later on I'll put in some code to actually process the text and insert semantic markup there.
         ...
         render_toc = fn[s][s.section && $('<li>').snippet(s).append($('<a href="##{s.section}">').text(s.section).append($('<ul>').append(map(render_toc, s.subsnippets || []))))],
     ...
     render_snippet = fn[s][s.section ? $('<section id="#{s.section}">').snippet(s).append($('<h#{s.level + 1}>').text(s.section)).append(
                     ...
                     s.role === 'pre' ? $('<pre>').snippet(s).text(outdent(s.text)) :
                  s.role === 'source' ? $('<pre class="source">').snippet(s).text(outdent(s.text)) :
                 s.role === 'prelude' ? $('<header>').append($('<h2 class="name">').text(s.name)).append($('<h3 class="author">').append(s.author)).append($('<p class="prelude">').text(s.text)) :
               s.role === 'enumerate' ? $('<ol start="#{s.start}">').snippet(s).append(map(fn[s][$('<li>').data('snippet', s).text(s)], s.items)) :
                                        $('<p>').snippet(s).text(s.text)],
                    ...
                    bind('keyup change', let[timeout = null] in
                                         fn_[let[v = $(this).val().replace(/\s+/g, ':')] in
                                             (timeout && clearTimeout(timeout),
                                              timeout = setTimeout(fn_[v.length ? $('.snippet').each(fn_[$(this).snippet().index[v] ? $(this).appear() : $(this).disappear()]) :
                                                                                  $('.snippet').appear()], 100))]))),
        $('#search input').focus()]);

  $.fn.snippet = fn[s][s ? this.data('snippet', s).addClass('snippet') : this.data('snippet')];
